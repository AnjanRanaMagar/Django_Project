...
def index(request):
    ...
    today = date.today()
    for their_age in student:
        their_age['Age'] = today.year - their_age['Dob'].year - (
                    (today.month, today.day) < (their_age['Dob'].month, their_age['Dob'].day))
        a = Student.objects.get(pk=their_age['id'])
        a.age = their_age['Age']
        a.save()
